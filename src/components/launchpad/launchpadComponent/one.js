...
import { checkNetwork, getNetwork } from "../../../Utils/initialFunctions";
...
const OneComponent = (props) => {
  ...
  const onContractInputChanged = async (event) => {
    ...
    if (window.web3.utils.isAddress(targetValue) === false) {
      ...
      const allowance = await window.TokenContract.methods
        .allowance(window.userAddress, window.presaleGeneratorAddress)
        .call();
  ...
  const handleApprove = async () => {
    ...
    await window.TokenContract.methods
      .approve(window.presaleGeneratorAddress, amount)
  ...
  const handleInputChanged = async (event) => {
    ...
    const targetValue = event.target.value;
    if (targetName === "listing") {
      if (targetValue === "true") window.autolist = true;
      else window.autolist = false;
    }
    setInfo1((info1) => ({
      ...info1,
