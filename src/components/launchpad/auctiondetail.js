...
const AuctionDetail = () => {
  ...
  useEffect(() => {
    const initialize = async() =>{
      if (window.web3.utils.isAddress(address) === true) {
        ...
        const tokenAddressCall = window.auctionContract.methods.S_TOKEN().call();
        const ownerCall = window.auctionContract.methods.PRESALE_OWNER().call();
        const payTokenCall = window.auctionContract.methods.B_TOKEN().call();
        const presaleinfoCall = window.auctionContract.methods
          .PRESALE_INFO()
          .call();
        const refundTypeCall = window.auctionContract.methods.returnType().call();
        const statusCall = window.auctionContract.methods.STATUS().call();
        const factoryAddressCall = window.auctionContract.methods
          .PRESALE_GENERATOR()
          .call();
        const myInfoCall = window.auctionContract.methods
          .BUYERS(window.userAddress)
          .call();
        const priceCall = window.auctionContract.methods
        .getPrice()
        .call();
        ...
        const tokenSupplyCall = window.tokenContract.methods.totalSupply().call();
        ...
        if(getNetwork().length > 0 )
        {
          ...
          setBuybtntext(`Buy with ${String(findPayToken(infos[1])).toUpperCase()}`);
          setPresaleSupply(
            precision.remove(
              infos[2].HARDCAP,
              SaleTokenAddresses[getNetwork()[0]][findPayToken(infos[1])].decimals
            ) /
              precision.remove(
                infos[2].START_PRICE,
                SaleTokenAddresses[getNetwork()[0]][findPayToken(infos[1])].decimals
        ...
        setLiquidity((precision.remove(infos[2].AMOUNT, SaleTokenAddresses[getNetwork()[0]][findPayToken(infos[1])].decimals) * infos[2].LIQUIDITY_PERCENT)/1000);
          ...
          datasets: [
            {
              ...
              data: [
                precision.remove(
                    ...
                    precision.remove(infos[2].AMOUNT, SaleTokenAddresses[getNetwork()[0]][findPayToken(infos[1])].decimals) * infos[2].LIQUIDITY_PERCENT/1000,
                    precision.remove(tokenInfo[1], Number(tokenInfo[3])) - precision.remove(infos[2].AMOUNT, SaleTokenAddresses[getNetwork()[0]][findPayToken(infos[1])].decimals) * infos[2].LIQUIDITY_PERCENT/1000 - precision.remove(
                        infos[2].HARDCAP,
                        SaleTokenAddresses[getNetwork()[0]][findPayToken(infos[1])].decimals
                      ) /
                        precision.remove(
  ...
  const handleBuyToken = async () => {
    ...
    await window.auctionContract.methods
      .userDeposit(
        precision.add(
          tokenAmount,
          SaleTokenAddresses[getNetwork()[0]][paytoken].decimals
      ...
      .on("receipt", async () => {
        setBuybtntext(`Buy with ${String(paytoken).toUpperCase()}`);
        const myPurchaseCall = await window.auctionContract.methods
          .BUYERS(window.userAddress)
          .call();
        setMypurchase(
          precision.remove(Number(myPurchaseCall.baseDeposited), SaleTokenAddresses[getNetwork()[0]][paytoken].decimals)
        );
      });
  };
            ...
            <Row>
              <div style={{ background: "#fff", padding: 20 }}>
                <div className="header">
                  <img
                    className="logo"
                    src={"https://www.pinkswap.finance/pinkswap.png"} alt="tokenlogo"
                  ></img>
